...
class Rwkv4NeoChannelMix(nn.Module):
    ...
    def forward(self, hidden_state, previous_input=None):
        ...
        xk = hidden_state * self.mix_k + sx * (1 - self.mix_k)
        xr = hidden_state * self.mix_r + sx * (1 - self.mix_r)
...
class Rwkv4NeoTimeMix(nn.Module):
    ...
    def forward(self, hidden_state, previous_state=None):
        ...
        # xk = torch.lerp(sx, hidden_state, self.k_mix)
        # xv = torch.lerp(sx, hidden_state, self.v_mix)
        # xr = torch.lerp(sx, hidden_state, self.r_mix)
        xk = hidden_state * self.mix_k + sx * (1 - self.mix_k)
        xv = hidden_state * self.mix_v + sx * (1 - self.mix_v)
        xr = hidden_state * self.mix_r + sx * (1 - self.mix_r)
        r = F.sigmoid(self.receptance(xr))
        k = self.key(xk)
        v = self.value(xv)
